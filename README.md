# Aprendiendo Python — Universidad Fidelitas

Este repositorio contiene ejercicios y prácticas de Python realizados durante el curso universitario en la Universidad Fidelitas.

## Descripción

Aquí encontrarás ejemplos básicos de programación en Python, organizados por semanas y ejercicios. El objetivo principal es practicar los conceptos fundamentales que se vieron en clase.

## Estructura del repositorio

- `Semana8/` - Ejercicios de estructuras de decisión y repetición.
- `Semana9/` - Ejercicios adicionales y prácticas de programación.
- `Semana10/` - Ejercicios de unidimensionales y otros problemas.
- `Semana11/` - Ejercicios de matrices bidimensionales.
- `Semana13/` - Caso evaluado y ejercicio de repaso.
- `ProyectoFinal/` - Proyecto final del curso.

## Tecnologías

- Python 3

## Uso

Cada archivo `.py` está dedicado a un ejercicio o práctica. Puedes ejecutar los scripts desde la terminal con:

```bash
python nombre_del_archivo.py
```

## Nota

Este repositorio es una colección de los temas básicos vistos en clase, ideal para repasar conceptos de programación en Python.
